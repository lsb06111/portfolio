...
$dTimeL = '23:59:59';
// 오늘(D)
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time())." {$dTimeF}' and '".date("Y-m-d",time())." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[0] = $row['cnt'];
$member_href_today = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time());

// D-1
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time()-$oneDay)." {$dTimeF}' and '".date("Y-m-d",time()-$oneDay)." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[1] = $row['cnt'];
$member_href_l1 = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time()-$oneDay);

// D-2
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time()-$oneDay*2)." {$dTimeF}' and '".date("Y-m-d",time()-$oneDay*2)." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[2] = $row['cnt'];
$member_href_l2 = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time()-$oneDay*2);

// D-3
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time()-$oneDay*3)." {$dTimeF}' and '".date("Y-m-d",time()-$oneDay*3)." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[3] = $row['cnt'];
$member_href_l3 = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time()-$oneDay*3);

// D-4
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time()-$oneDay*4)." {$dTimeF}' and '".date("Y-m-d",time()-$oneDay*4)." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[4] = $row['cnt'];
$member_href_l4 = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time()-$oneDay*4);

// D-5
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time()-$oneDay*5)." {$dTimeF}' and '".date("Y-m-d",time()-$oneDay*5)." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[5] = $row['cnt'];
$member_href_l5 = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time()-$oneDay*5);

// D-6
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-d",time()-$oneDay*6)." {$dTimeF}' and '".date("Y-m-d",time()-$oneDay*6)." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt[6] = $row['cnt'];
$member_href_l6 = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m-d",time()-$oneDay*6);

// 금월
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-m-01",time())." {$dTimeF}' and '".date("Y-m-d",time())." {$dTimeL}' ";
$row = sql_fetch($sql);
$member_cnt['month'] = $row['cnt'];
$member_href_thismonth = G5_ADMIN_URL."/member_list.php?sfl=mb_datetime&stx=".date("Y-m",time());

// 금년
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_datetime between '".date("Y-01-01",time())." {$dTimeF}' and '".date("Y-m-d",time())." {$dTimeL}' ";
$row = sql_fetch($sql);
